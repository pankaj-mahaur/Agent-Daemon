...
> **Cross-platform note:** Commands below use PowerShell syntax. On Unix/macOS, substitute `Remove-Item` with `rm`, `New-Item -ItemType Directory -Force` with `mkdir -p`, `Out-File -Encoding utf8` with `>`, and `$env:VAR` with `export VAR=`.
